...
citation P / R     84.4 / 96.4   82.3 / 97.0    82.1 / 97.0
  by logical fact  87.5 / 96.4   92.4 / 98.5    92.3 / 98.5
```
...
### A defect in the grounding rule itself

A plan is not required to fill a slot's `entity`, and `SourceLabelGrounding`
looked a missing one up as the *empty* entity -- which matches no fact, and so
read as "the source does not have this row" when the source had it for exactly
one filer. Twelve blocked cases had a null slot entity, and every one of them
was refused for a reason that was not about the source at all.

`facts_for_label` asks the store the question the caller meant -- which facts
carry this label, whichever filer filed it -- and the determinacy requirement
