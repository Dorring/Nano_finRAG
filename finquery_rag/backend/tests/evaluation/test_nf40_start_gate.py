...
def test_nf40_accepts_verified_r2_snapshot(tmp_path):
    ...
    require_verified_nf39_r2_inputs(acceptance_path=tmp_path / "acceptance.json", snapshot_manifest_path=tmp_path / "snapshot.json", frozen_payload_path=payload, expected_payload_sha256=hashlib.sha256(payload.read_bytes()).hexdigest())
...
def test_nf40_rejects_payload_hash_mismatch(tmp_path):
    ...
    with pytest.raises(EvaluationIntegrityError):
        require_verified_nf39_r2_inputs(acceptance_path=tmp_path / "acceptance.json", snapshot_manifest_path=tmp_path / "snapshot.json", frozen_payload_path=payload, expected_payload_sha256=hashlib.sha256(b"frozen").hexdigest())
