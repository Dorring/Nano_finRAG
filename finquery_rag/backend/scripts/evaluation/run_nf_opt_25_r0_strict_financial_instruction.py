"""NF-OPT-25 R0: strict financial evidence instruction calibration.

This is a one-variable instruction ablation over the frozen SADA-V1 Top100,
original query bytes, Statement-Aware Evidence Unit V1, and Qwen3-4B scoring
contract.  Gold-dependent analysis is isolated after the prediction seal.
"""
...
BASE_COMMIT = "dd1b64ca0d11b0f20c2ceb6096bb1e39bb68470e"
OUT_NAME = "nf-opt-25-r0-strict-financial-instruction"
...
STRICT_FINANCIAL_INSTRUCTION = (
    "Given a financial question and an evidence candidate, judge whether the candidate "
    "provides direct evidence needed to answer the question.\n\n"
    "Prioritize exact agreement on the financial metric, reporting period, financial "
    "statement or row context, and required operand evidence.\n\n"
    "Prefer direct financial evidence over merely related discussion, commentary, or "
    "semantically similar but indirect evidence."
)
...
def load_inputs(backend_root: Path, nf23: Any, out: Path) -> dict[str, Any]:
    ...
    original_query_mismatch = 0
    for case_id, ranked in sada.items():
        expected_query_sha = inputs["qviews"][case_id].get("main_query_view_sha256")
        if any(item.get("query_view_sha256") != expected_query_sha for item in ranked):
            original_query_mismatch += 1
    if original_query_mismatch:
        raise RuntimeError(f"frozen Original Query mismatch count={original_query_mismatch}")
    ...
    write_json(out / "frozen-sada-contract.json", {
        "gate": "NF-OPT-25-R0", "source": TOP100_SADA_REL, "prediction_sha256": sada_sha, "prediction_seal_sha256": sha256_file(seal_path), "candidate_budget": 100, "queries": 72, "strict_top100_supply": "78/80", "candidate_mismatch": 0, "original_query_mismatch": 0, "admission_rerun": False, "gold_reads_before_experimental_prediction_seal": 0,
    })
    write_json(out / "frozen-candidate-contract.json", {
        "candidate_source": "frozen_sada_v1_top100", "candidate_budget": 100, "queries": 72, "pairs": 7200, "candidate_identity_unchanged": True, "sada_prediction_sha256": sada_sha, "sada_top100_strict_supply": "78/80", "candidate_mismatch": 0, "original_query_mismatch": 0, "statement_aware_candidate_bytes_frozen": True, "gold_reads": 0,
    })
...
def build_candidate_rows(backend_root: Path, nf23: Any, loaded: dict[str, Any], out: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    write_json(out / "serialization-seal.json", {"gate": "NF-OPT-25-R0", "records": len(manifest_rows), "queries": 72, "pairs": 7200, "serialization_sha256": serialization_sha, "candidate_serialization_match": True, "nf23_manifest_overlap_checked": len(expected_manifest), "mismatches": 0, "gold_reads": 0, "sealed": True})
...
def frozen_reranker_contract(backend_root: Path, nf23: Any, out: Path) -> dict[str, Any]:
    ...
    write_json(out / "frozen-reranker-contract.json", {"gate": "NF-OPT-25-R0", "model": contract["model_id"], "model_revision": contract["revision"], "dtype": contract["dtype"], "max_length": contract["max_length"], "batch_size": contract["batch_size"], "scoring": contract["scoring"], "baseline_instruction_sha256": contract["instruction_sha256"], "strict_instruction_sha256": sha256_text(STRICT_FINANCIAL_INSTRUCTION), "instruction_unchanged": False, "only_instruction_differs": True, "original_query_unchanged": True, "query_candidate_contract": "same Original Query bytes and Statement-Aware V1 candidate bytes", "retrieval_rerun": False, "admission_rerun": False, "instruction_locked": True})
    return contract
...
def prepare_runtime_rows(candidate_rows: list[dict[str, Any]], qviews: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    runtime_rows: list[dict[str, Any]] = []
    for row in candidate_rows:
        case_id = row["case_id"]
        query = qviews[case_id]["main_query_view"]
        query_sha = qviews[case_id]["main_query_view_sha256"]
        runtime_rows.append({"case_id": case_id, "original_query": query, "original_query_sha256": query_sha, "candidates": row["candidates"]})
    return runtime_rows
...
def postseal_nf25(
    ...
    baseline_instruction_sha: str,
    strict_instruction_sha: str,
) -> dict[str, Any]:
    """Score and audit NF-OPT-25 only after the experimental seal."""
    root = backend_root / "artifacts/evaluation"
    # Reuse the frozen, Gold-dependent metric helpers only after prediction
    # sealing.  The compatibility payload is never used by the scoring path.
    fake_requirements = {
        "requirements": {
            case_id: {
                "original_question": loaded["inputs"]["qviews"][case_id]["main_query_view"],
                "target_terms": [],
                "explicit_periods": [],
                "operation": None,
                "required_slots": [],
            }
            for case_id in loaded["inputs"]["qviews"]
        },
        "audit": {"at_least_one_structured_requirement": 0, "target_terms": 0, "explicit_periods": 0, "operation": 0, "required_slots": 0},
    }
    old_result = postseal(backend_root, out, loaded, fake_requirements, runtime)
    nf21 = load_nf21(backend_root)
    strict_rows = read_jsonl(root / "pdf-retrieval-v4-strict-source-contract/strict-gold-source-bindings.jsonl")
    prediction_rows = read_gzip_jsonl(out / "predictions.jsonl.gz")
    experiment = {row["case_id"]: sorted(row["ranked_candidates"], key=lambda item: int(item["post_rerank_rank"])) for row in prediction_rows}
    ...
    def remap(variants: dict[str, Any]) -> dict[str, Any]:
        return {
            "sada_sa_baseline": variants["sada_original_query"],
            "strict_financial_instruction_v1": variants["query_requirement"],
        }

    strict = remap(old_result["strict"])
    semantic = remap(old_result["semantic"])
    write_json(out / "strict-metrics.json", {"strict_sources": len(strict_rows), **strict})
    write_json(out / "semantic-metrics.json", semantic)

    movement_payload = read_json(out / "rank-movement.json")
    movement_rows = movement_payload["rows"]
    improved = sum(row["rank_delta"] > 0 for row in movement_rows)
    worsened = sum(row["rank_delta"] < 0 for row in movement_rows)
    unchanged = sum(row["rank_delta"] == 0 for row in movement_rows)
    write_json(out / "rank-movement.json", {
        "baseline": "SADA-SA-BASELINE",
        "experiment": "Strict Financial Evidence Instruction V1",
        "rescued": movement_payload["rescued"],
        "damaged": movement_payload["damaged"],
        "net": movement_payload["net"],
        "gold_rank_improved": improved,
        "gold_rank_unchanged": unchanged,
        "gold_rank_worsened": worsened,
        "rows": movement_rows,
    })

    top10 = read_json(out / "top10-to-top5-analysis.json")
    top10["baseline"] = "SADA-SA-BASELINE"
    top10["experiment"] = "Strict Financial Evidence Instruction V1"
    write_json(out / "top10-to-top5-analysis.json", top10)

    failure_records = load_failure_records(root)
    rank_by_variant = {
        "baseline": {case: rank_map(rows) for case, rows in baseline.items()},
        "instruction": {case: rank_map(rows) for case, rows in experiment.items()},
    }
    failure_tags = (
        "wrong_period",
        "same_metric_wrong_row",
        "multi_slot_competition",
        "calculation_operand_competition",
        "same_document_wrong_evidence",
    )
    cohort_outputs: dict[str, dict[str, Any]] = {}
    for tag in failure_tags:
        ids = {(row.get("case_id"), row.get("source_index", 0)) for row in failure_records if tag in (row.get("tags") or [])}
        rows = [row for row in movement_rows if (row["case_id"], row.get("source_index", 0)) in ids]
        cohort_outputs[tag] = {
            "cases": len(rows),
            "rescued": sum(row["outcome"] == "rescued" for row in rows),
            "damaged": sum(row["outcome"] == "damaged" for row in rows),
            "improved": sum(row["rank_delta"] > 0 for row in rows),
            "unchanged": sum(row["rank_delta"] == 0 for row in rows),
            "worsened": sum(row["rank_delta"] < 0 for row in rows),
        }
    write_json(out / "wrong-period-analysis.json", cohort_outputs["wrong_period"])
    write_json(out / "same-metric-row-analysis.json", cohort_outputs["same_metric_wrong_row"])
    write_json(out / "multi-slot-analysis.json", cohort_outputs["multi_slot_competition"])
    write_json(out / "calculation-operand-analysis.json", cohort_outputs["calculation_operand_competition"])
    write_json(out / "same-document-evidence-analysis.json", cohort_outputs["same_document_wrong_evidence"])
    ...
    def cohort_summary(ids: set[tuple[Any, Any]]) -> dict[str, Any]:
        rows = [row for row in movement_rows if (row["case_id"], row.get("source_index", 0)) in ids]
        deltas = [row["rank_delta"] for row in rows]
        return {
            "total": len(rows),
            "baseline_r5": sum(row["baseline_rank"] <= 5 for row in rows),
            "instruction_r5": sum(row["experimental_rank"] <= 5 for row in rows),
            "rescued": sum(row["outcome"] == "rescued" for row in rows),
            "damaged": sum(row["outcome"] == "damaged" for row in rows),
            "mean_rank_delta": statistics.mean(deltas) if deltas else None,
            "median_rank_delta": statistics.median(deltas) if deltas else None,
        }

    write_json(out / "near-boundary-clear-loss-analysis.json", {"near_boundary": cohort_summary(near_ids), "clear_loss": cohort_summary(clear_ids)})

    type_aliases = {
        "table_row_backed": "table-row",
        "table_cell_backed": "table-fragment",
        "table_fragment_backed": "table-fragment",
    ...
    for binding in strict_rows:
        unit = loaded["units"].get(binding["candidate_key"], {})
        label = type_aliases.get(unit.get("candidate_type", "unresolved"), "unresolved")
        by_type[label].append(binding)
    representation = {}
    for label in ("table-row", "table-fragment", "mixed", "narrative", "unresolved"):
        bindings = by_type.get(label, [])
        representation[label] = {
            "gold_bindings": len(bindings),
            "baseline_r5": nf21.strict_metrics(bindings, baseline)["@5"],
            "instruction_r5": nf21.strict_metrics(bindings, experiment)["@5"],
            "gain_sources": nf21.strict_metrics(bindings, experiment)["@5"]["hits"] - nf21.strict_metrics(bindings, baseline)["@5"]["hits"],
        }
    write_json(out / "representation-type-analysis.json", representation)

    score_rows: list[dict[str, Any]] = []
    for binding in strict_rows:
        case_id, key = binding["case_id"], binding["candidate_key"]
        base_items, exp_items = baseline[case_id], experiment[case_id]
        base_by_key = {item["candidate_key"]: item for item in base_items}
        exp_by_key = {item["candidate_key"]: item for item in exp_items}
        base_scores = [float(item.get("qwen_statement_score", 0.0)) for item in base_items]
        exp_scores = [float(item.get("qwen_statement_score", 0.0)) for item in exp_items]
        base_gold = base_by_key.get(key, {})
        exp_gold = exp_by_key.get(key, {})
        score_rows.append({
            "case_id": case_id,
            "candidate_key": key,
            "baseline_rank": rank_by_variant["baseline"][case_id].get(key, 10**9),
            "instruction_rank": rank_by_variant["instruction"][case_id].get(key, 10**9),
            "baseline_top1_score": base_scores[0],
            "baseline_top5_cutoff_score": base_scores[4],
            "baseline_gold_score": float(base_gold.get("qwen_statement_score", 0.0)),
            "baseline_gold_minus_cutoff": float(base_gold.get("qwen_statement_score", 0.0)) - base_scores[4],
            "instruction_top1_score": exp_scores[0],
            "instruction_top5_cutoff_score": exp_scores[4],
            "instruction_gold_score": float(exp_gold.get("qwen_statement_score", 0.0)),
            "instruction_gold_minus_cutoff": float(exp_gold.get("qwen_statement_score", 0.0)) - exp_scores[4],
        })
    write_json(out / "score-distribution-analysis.json", {
        "records": len(score_rows),
        "baseline": {
            "top1_score": {f"p{q}": percentile([row["baseline_top1_score"] for row in score_rows], q / 100) for q in (25, 50, 75)},
            "top5_cutoff_score": {f"p{q}": percentile([row["baseline_top5_cutoff_score"] for row in score_rows], q / 100) for q in (25, 50, 75)},
            "gold_minus_cutoff": {f"p{q}": percentile([row["baseline_gold_minus_cutoff"] for row in score_rows], q / 100) for q in (25, 50, 75)},
        },
        "instruction": {
            "top1_score": {f"p{q}": percentile([row["instruction_top1_score"] for row in score_rows], q / 100) for q in (25, 50, 75)},
            "top5_cutoff_score": {f"p{q}": percentile([row["instruction_top5_cutoff_score"] for row in score_rows], q / 100) for q in (25, 50, 75)},
            "gold_minus_cutoff": {f"p{q}": percentile([row["instruction_gold_minus_cutoff"] for row in score_rows], q / 100) for q in (25, 50, 75)},
        },
        "rows": score_rows,
    })

    direct_indirect = {}
    for tag in ("same_document_wrong_evidence", "generic_financial_similarity"):
        ids = {(row.get("case_id"), row.get("source_index", 0)) for row in failure_records if tag in (row.get("tags") or [])}
        rows = [row for row in movement_rows if (row["case_id"], row.get("source_index", 0)) in ids]
        direct_indirect[tag] = {"cases": len(rows), "mean_rank_delta": statistics.mean([row["rank_delta"] for row in rows]) if rows else None, "improved": sum(row["rank_delta"] > 0 for row in rows), "worsened": sum(row["rank_delta"] < 0 for row in rows)}
    write_json(out / "direct-indirect-evidence-analysis.json", direct_indirect)

    no_answer = read_json(out / "no-answer-analysis.json")
    no_answer["baseline_variant"] = "SADA-SA-BASELINE"
    no_answer["instruction_variant"] = "Strict Financial Evidence Instruction V1"
    no_answer["threshold_modified"] = False
    write_json(out / "no-answer-analysis.json", no_answer)

    strict_r5 = strict["strict_financial_instruction_v1"]["@5"]["hits"]
    strict_r10 = strict["strict_financial_instruction_v1"]["@10"]["hits"]
    strict_r20 = strict["strict_financial_instruction_v1"]["@20"]["hits"]
    strict_r50 = strict["strict_financial_instruction_v1"]["@50"]["hits"]
    strict_r100 = strict["strict_financial_instruction_v1"]["@100"]["hits"]
    semantic_r5 = semantic["strict_financial_instruction_v1"]["@5"]["hits"]
    baseline_semantic_r5 = semantic["sada_sa_baseline"]["@5"]["hits"]
    multi = remap(old_result["multi"])
    calc = remap(old_result["calc"])
    multi_all5 = multi["strict_financial_instruction_v1"]["@5"]["all"]
    baseline_multi_all5 = multi["sada_sa_baseline"]["@5"]["all"]
    calc_all5 = calc["strict_financial_instruction_v1"]["@5"]["all_slots"]
    baseline_calc_all5 = calc["sada_sa_baseline"]["@5"]["all_slots"]
    write_json(out / "multi-evidence-analysis.json", {"denominator": 16, **multi})
    write_json(out / "calculation-slot-analysis.json", {"denominator": 11, **calc})
    rescued, damaged = movement_payload["rescued"], movement_payload["damaged"]
    if strict_r5 >= 53 and rescued - damaged >= 7 and damaged <= 4 and strict_r10 >= 60 and strict_r20 >= 68 and strict_r50 >= 72 and strict_r100 == 78 and semantic_r5 >= baseline_semantic_r5 and multi_all5 >= baseline_multi_all5 and calc_all5 >= baseline_calc_all5:
        effective, next_gate = True, "internal_retrieval_shadow_freeze_review"
    elif 50 <= strict_r5 <= 52 and rescued > damaged and rescued - damaged >= 4 and strict_r10 >= 59 and strict_r100 == 78 and semantic_r5 >= baseline_semantic_r5 and multi_all5 >= baseline_multi_all5 and calc_all5 >= baseline_calc_all5:
        effective, next_gate = "marginal", "internal_retrieval_shadow_freeze_review"
    else:
        effective, next_gate = False, "internal_retrieval_method_freeze"
    decision = {
        "gate": "NF-OPT-25-R0",
        "evaluation_role": "development_shadow_reranker_instruction_calibration",
        ...
        "original_query_unchanged": True,
        "statement_aware_contract_unchanged": True,
        "baseline_instruction_sha": baseline_instruction_sha,
        "strict_instruction_sha": strict_instruction_sha,
        "baseline_r5_hits": strict["sada_sa_baseline"]["@5"]["hits"],
        "baseline_r10_hits": strict["sada_sa_baseline"]["@10"]["hits"],
        "baseline_r20_hits": strict["sada_sa_baseline"]["@20"]["hits"],
        "baseline_r50_hits": strict["sada_sa_baseline"]["@50"]["hits"],
        "baseline_r100_hits": strict["sada_sa_baseline"]["@100"]["hits"],
        "strict_instruction_r5_hits": strict_r5,
        "strict_instruction_r10_hits": strict_r10,
        "strict_instruction_r20_hits": strict_r20,
        "strict_instruction_r50_hits": strict_r50,
        "strict_instruction_r100_hits": strict_r100,
        "rescued": rescued,
        "damaged": damaged,
        "net": rescued - damaged,
        "top10_to_top5_promoted": top10["promoted_to_top5"],
        "semantic_r5_hits": semantic_r5,
        "multi_evidence_all_at_5": multi_all5,
        "calculation_all_slots_at_5": calc_all5,
        "strict_financial_instruction_effective": effective,
        "selected_internal_shadow_method": "sada_statement_aware_strict_instruction_v1" if effective is not False else "sada_statement_aware_v1",
    ...
    write_json(out / "decision.json", decision)
    return {"strict": strict, "semantic": semantic, "multi": multi, "calc": calc, "movement": movement_payload, "decision": decision}
...
def main() -> int:
    ...
    baseline_instruction = contract["instruction"]
    baseline_instruction_sha = sha256_text(baseline_instruction)
    strict_instruction_sha = sha256_text(STRICT_FINANCIAL_INSTRUCTION)
    if baseline_instruction_sha != contract["instruction_sha256"]:
        raise RuntimeError("baseline instruction hash mismatch")
    (out / "baseline-instruction.txt").write_bytes(baseline_instruction.encode("utf-8"))
    (out / "baseline-instruction.sha256").write_text(baseline_instruction_sha + "\n", encoding="utf-8")
    (out / "strict-financial-instruction-v1.txt").write_bytes(STRICT_FINANCIAL_INSTRUCTION.encode("utf-8"))
    (out / "strict-financial-instruction-v1.sha256").write_text(strict_instruction_sha + "\n", encoding="utf-8")
    write_json(out / "instruction-contract.json", {
        "gate": "NF-OPT-25-R0",
        "baseline_instruction_sha": baseline_instruction_sha,
        "strict_instruction_sha": strict_instruction_sha,
        "strict_instruction_text": STRICT_FINANCIAL_INSTRUCTION,
        "instruction_locked": True,
        "only_instruction_differs": True,
        "post_hoc_editing": False,
    })
    candidate_rows, serialization_meta = build_candidate_rows(backend_root, nf23, loaded, out)
    query_manifest = [{"case_id": case_id, "original_query_sha256": loaded["inputs"]["qviews"][case_id]["main_query_view_sha256"]} for case_id in sorted(loaded["inputs"]["qviews"])]
    query_manifest_sha = sha256_text(json.dumps(query_manifest, sort_keys=True, separators=(",", ":")))
    runtime_path = out / "runtime-metrics.json"
    prediction_path = out / "predictions.jsonl.gz"
    if args.postseal:
        runtime = read_json(runtime_path)
    else:
        runtime_rows = prepare_runtime_rows(candidate_rows, loaded["inputs"]["qviews"])
        write_json(out / "input-manifest.json", {"queries": 72, "pairs": 7200, "candidate_budget": 100, "original_query_manifest_sha256": query_manifest_sha, "serialization_sha256": serialization_meta["serialization_sha256"], "candidate_identity_mismatch": 0, "original_query_mismatch": 0, "gold_reads": 0})
        runtime = score_experiment(runtime_rows, STRICT_FINANCIAL_INSTRUCTION, contract["revision"], contract["max_length"], prediction_path)
        write_json(runtime_path, {**runtime, "baseline_instruction_sha": baseline_instruction_sha, "strict_instruction_sha": strict_instruction_sha, "original_query_manifest_sha256": query_manifest_sha, "candidate_serialization_sha256": serialization_meta["serialization_sha256"]})
        ...
        write_json(out / "prediction-seal.json", {"gate": "NF-OPT-25-R0", "prediction_sha256": prediction_sha, "queries": 72, "pairs": 7200, "candidate_identity_mismatch": 0, "candidate_budget": 100, "original_query_manifest_sha256": query_manifest_sha, "statement_aware_serialization_sha256": serialization_meta["serialization_sha256"], "baseline_instruction_sha": baseline_instruction_sha, "strict_instruction_sha": strict_instruction_sha, "gold_reads_before_prediction_seal": 0, "sealed": True})
    if not prediction_path.exists():
        raise RuntimeError("prediction seal missing")
    result = postseal_nf25(backend_root, out, loaded, runtime, baseline_instruction_sha, strict_instruction_sha)
    write_json(out / "runtime-metrics.json", {**runtime, "baseline_instruction_sha": baseline_instruction_sha, "strict_instruction_sha": strict_instruction_sha, "original_query_manifest_sha256": query_manifest_sha, "candidate_serialization_sha256": serialization_meta["serialization_sha256"]})
    (out / "README.md").write_text("# NF-OPT-25 R0\n\nStrict Financial Evidence Instruction V1 is a single instruction-objective ablation over the frozen SADA-V1 Top100, Original Query bytes, Statement-Aware Evidence Unit V1, and Qwen3-Reranker-4B contract. The instruction is sealed before scoring; Gold is loaded only after prediction-seal.json. No retrieval, admission rerun, query rewrite, instruction sweep, or production switch is performed.\n", encoding="utf-8")
