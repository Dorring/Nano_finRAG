...
def run(args: argparse.Namespace) -> int:
    ...
    baseline_parity = len(baseline_hits) == 13
    _write(args.out_dir / "strict-source-change-report.json", {"authoritative_frozen_baseline_hit_count": 13, "replayed_baseline_strict_hit_count": len(baseline_hits), "baseline_parity": baseline_parity, "e1_strict_hit_count": len(e1_hits), "new_vs_replayed_baseline_gold_source_count": len(new_hits), "regressed_vs_replayed_baseline_gold_source_count": len(regressions), "new_vs_replayed_baseline_gold_sources": [{"case_id": case_id, "candidate_key": key} for case_id, key in sorted(new_hits)], "regressed_vs_replayed_baseline_gold_sources": [{"case_id": case_id, "candidate_key": key} for case_id, key in sorted(regressions)], "authoritative_original_13_retention": "not_scored_due_to_baseline_replay_parity_failure"})
