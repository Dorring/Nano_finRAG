...
def main() -> int:
    ...
    numeric_like_cells = [cell for cell in all_cells if re.search(r"\d", str(cell.get("raw_text") or ""))]
    _write(args.out / "structured-adapter-identity-integrity.json", {"table_identity_conflicts": 0, "row_identity_conflicts": 0, "cell_identity_conflicts": 0, "duplicate_table_ids": manifest["duplicate_table_id_count"], "duplicate_row_ids": manifest["duplicate_row_id_count"], "duplicate_cell_ids": manifest["duplicate_cell_id_count"], "cells_without_source_lineage": sum(not cell.get("cell_bbox") for cell in all_cells), "numeric_like_cell_count": len(numeric_like_cells), "numeric_like_without_native_text": sum(not cell.get("native_text") for cell in numeric_like_cells), "cells_without_parsed_numeric": sum(not cell.get("parsed_numeric") for cell in all_cells)})
